...
class RawDataSmoother {
    ...
    // Another example, you have missed a few packets, and you only got packets corresponding to time:
...
class NoiseCalculator implements INoiseAlgorithm {
    ...
    public void CalculateNoise(List<RawData> rawData) {
        ...
        int i;
        for (i=0 ; i < 10000 /*rawData.size()*/; i++) { //???
            System.out.println("before i = "+ i + " object = " +  System.identityHashCode(rawData.get(i)));
            ...
            System.out.println("After i = "+ i + " object = " +  System.identityHashCode(rawData.get(i)) + " noise = " + rawData.get(i).noise_quantity2);
        }
    }

    // rawData list of points, should already be good. i some point in the list, practically, it's last.
    // intersting point is a point that exists, and should be read...
    private void CalculateNoise(List<RawData> rawData, int intertintPoint) {
        assert rawData.size() > intertintPoint;
        // Find the list of 10 points that match the same 

        for(int k = 0; k < NOISE_PERIOD + 1; k++)
            assert rawData.get(k) != null;

        double []diffs = new double[NOISE_PERIOD ];
        // the points are [i-NOISE_PERIOD, i]
        // Are they 5 minutes apart?
        int k=0;
        for (int j=intertintPoint-(NOISE_PERIOD ); j <intertintPoint ; j++, k++) {

            if(!followingReadings(rawData.get(j), rawData.get(j+1))) {
                rawData.get(intertintPoint).noise_level = -1;
                assert false;
                return;
            }
            System.err.println("calculating difs j = " + j + " k = " + k);
            // calculate the difference vector
            diffs[k] = rawData.get(j).raw_value - rawData.get(j+1).raw_value;

        }
        assert k == NOISE_PERIOD; // Make sure that we have calculated the diffs over NOISE_PERIOD points. 
        ...
        System.err.println("i = " + intertintPoint);
        rawData.get(intertintPoint).noise_quantity1 = noiseTotal1 / (NOISE_PERIOD -1);
        rawData.get(intertintPoint).noise_level = levelFromTotal(rawData.get(intertintPoint).noise_quantity1);

        // now for the score method 2: sum of diff of the diffs. 
        double noiseTotal2 = 0;
        for(int j=0; j < NOISE_PERIOD - 1; j++) {
            noiseTotal2 += Math.abs(diffs[j] - diffs[j+1]);
        }
        System.out.println("Setting data for point i" +intertintPoint + " " + System.identityHashCode(rawData.get(intertintPoint)) + " noise = " + noiseTotal2);
        rawData.get(intertintPoint).noise_quantity2 = noiseTotal2 / (NOISE_PERIOD -1);
    }
